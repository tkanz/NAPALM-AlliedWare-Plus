...
class IOSDriver(NetworkDriver):
    ...
    def get_facts(self):
        ...
        # default values.
        vendor = u'Allied Telesis'
        uptime = -1
        serial_number, fqdn, os_version, hostname, domain_name = ('Unknown',) * 5

        # AWP obtain output from device
        show_ver = self._send_command('show system')
        show_hosts = self._send_command('show hosts')
        show_ip_dom = self._send_command('show ip domain-name')
        show_ip_int_br = self._send_command('show ip interface brief')
        show_serial = self._send_command('show system serialnumber')
        show_hostname = self._send_command('show run | i hostname')

        # AWP uptime/version/serial
        for line in show_ver.splitlines():
            if re.search (r"Uptime             : ", line):
                try:
                    _, uptime = line.split("Uptime             : ")
                except ValueError:
                    _, uptime = re.split(r"[.*?\], ", line)
            elif re.search(r"Uptime             : ", line):
                _, uptime = line.split("Uptime             : ")
                uptime = self.parse_uptime(uptime_str)

            if re.search(r"Current software   : ", line):
                try:
                    _, os_version = line.split("Current software   : ")
                except ValueError:
                    _, os_version = re.split(r"[.*?\], ", line)
            elif re.search(r"Software version   : ", line):
                _, os_version = line.split("Software version   : ")

        # AWP Determine domain_name and fqdn
        for line in show_hosts.splitlines():
            if re.search(r"Default domain is ", line):
                try:
                    _, domain_name = line.split("Default domain is ")
                except ValueError:
                    _, domain_name = re.split(r"[.*?\], ", line)
            elif re.search(r"Default domain is ", line):
                _, domain_name = line.split("Default domain is ")
            domain_name = domain_name.strip()

        # AWP host name
        for line in show_hostname.splitlines():
            if re.search(r"hostname ", line):
                try:
                    _, hostname = line.split("hostname ")
                except ValueError:
                    _, hostname = re.split(r"[.*?\], ", line)
        ...
        # AWP model filter
        try:
            match_model = re.search(r"Base.*(x...-....)", show_ver, flags=re.IGNORECASE)
            model = match_model.group(1)
        except AttributeError:
            model = u'Unknown'

        # AWP serial number
        serial_number = show_serial
    ...
    def get_interfaces(self):
        ...
        for line in command.splitlines():
            ...
            for pattern in (interface_regex_1, interface_regex_2):
                ...
                if interface_match:
                    if pattern == interface_regex_1:
                        interface = interface_match.group(2)
                    if pattern == interface_regex_2:
                        protocol = interface_match.group(2)
                        status = interface_match.group(4)
            ...
            mac_addr_regex = r"(address.is.({}))".format(MAC_REGEX)
            if re.search(mac_addr_regex, line):
                mac_addr_match = re.search(mac_addr_regex, line)
                mac_address = napalm.base.helpers.mac(mac_addr_match.groups()[1])
            ...
            speed_regex = r"current.speed.([0-9]+)"
